...
class LSTMCrossAttention(nn.Module):
    "LSTM+Multi-layer Cross Attention Mechanism"
    def __init__(self, input_dim, n_heads, num_time_slots, attn_drop, residual):
        super(LSTMCrossAttention, self).__init__()

        self.LSTM0 = nn.LSTM(input_size=input_dim,hidden_size=32,num_layers=1)
        self.LSTM1 = nn.LSTM(input_size=input_dim,hidden_size=32,num_layers=1)
    ...
    def forward(self, inputs0, inputs1):
        # inputs0 = torch.tensor(self.LSTM0(inputs0))
        # inputs1 = torch.tensor(self.LSTM1(inputs1))
        inputs0 = self.LSTM0(inputs0)[0]
        inputs1 = self.LSTM1(inputs1)[0]
        print("inputs:",type(inputs0), inputs1,"over")
        # 1. add position embeddings
        position_inputs = torch.arange(0, self.num_time_slots).reshape(1, -1).repeat(inputs0.shape[0], 1).long()
        # [N,T]  [[0,1,2..,time-1],[0,1,2,..,time-1],...]
        temporal_inputs0 = inputs0 + self.position_embeddings[position_inputs]  # [N,T,F]
        temporal_inputs1 = inputs1 + self.position_embeddings[position_inputs]

        # 2: Query, Key based multi-head self attention.
        q = torch.tensordot(temporal_inputs1, self.Q_embedding_weights, dims=([2], [0]))  # [N,T,F]
        k = torch.tensordot(temporal_inputs0, self.K_embedding_weights, dims=([2], [0]))
        v = torch.tensordot(temporal_inputs0, self.V_embedding_weights, dims=([2], [0]))
        ...
        masks = tril.repeat(outputs.shape[0], 1, 1).to(inputs0.device)  # [h*N, T, T]
        ...
        if self.residual:
            outputs = outputs + temporal_inputs0
        return outputs
